(() => {
  ...
  function stableTimelineBadgeHost(image, styleOf = (node) => globalThis.getComputedStyle?.(node)) {
    const photo = image?.closest?.('[data-testid="tweetPhoto"]');
    if (!photo) return null;
    let node = image?.parentElement ?? null;
    while (node && photo.contains(node)) {
      let position = "";
      try { position = String(styleOf?.(node)?.position || ""); } catch {}
  ...
  function installGallery() {
    ...
    function syncTimelineSavedBadge(image, isSaved) {
      ...
      // Use only a positioning context already owned by X. Never change layout
      // properties on tweetPhoto or its ancestors just to place the badge.
      const host = stableTimelineBadgeHost(image);
      if (!host) {
        removeTimelineSavedBadge(image);
        return;
      }
      ...
      if (!badge || badge.parentElement !== host) {
        ...
        badge.textContent = "\u2713";
        badge.setAttribute("aria-label", "Lakomics 서버 저장됨");
        host.append(badge);
    ...
    function syncTimelineSavedImage(image, onlyMediaUrl = null) {
      ...
      // The badge uses an existing X positioning context and never rewrites the
      // media container's layout styles.
  ...
  function runtimeMessage(message) {
    return new Promise((resolve) => {
      ...
      try {
        // Chromium/Titanium may return Promise<undefined> before the callback.
        // Callback-form sendMessage owns the result here.
  ...
  function createGalleryUi() {
    ...
    root.innerHTML = `
      <style>
        ...
        #${GALLERY_ROOT_ID} .lakomics-x-gallery-card.is-saved { box-shadow: inset 0 0 0 2px rgba(96,126,103,.72); }
        #${GALLERY_ROOT_ID} .lakomics-x-gallery-saved-badge { position: absolute; top: 8px; right: 8px; z-index: 2; display: none; width: 30px; height: 30px; align-items: center; justify-content: center; padding: 0; border: 1px solid #819487; border-radius: 3px; background: #607e67; color: #f4f1e7; box-shadow: 0 3px 10px rgba(0,0,0,.34); font-size: 20px; font-weight: 900; line-height: 1; pointer-events: none; }
