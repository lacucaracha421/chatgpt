...
test("first-level visibility is structural: roots show and non-roots require a pin", () => {
  const current = [{ id:"reverse-root", kind:"root", name:"Reverse", parentId:null }, { id:"game", kind:"root", name:"Game", parentId:null }, { id:"reverse-tag", kind:"tag", name:"Reverse", parentId:"game" }];
  const layout = layoutApi.resetLayout(current);
  assert.deepEqual(plain(layoutApi.getPinnedLevel(current, layout, [], 0).slots.filter(Boolean).map(e=>e.id)), ["reverse-root","game"]);
  assert.ok(layoutApi.getPinnedLevel(current, layout, ["reverse-tag"], 0).slots.filter(Boolean).some(e=>e.id==="reverse-tag"));
});
