(() => {
  ...
  function mount({ entries, layout, pinnedIds, hiddenIds, origin, onSave, onClose }) {
    ...
    function close() {
      if (disposed) return;
      disposed = true; host.remove(); window.removeEventListener('resize',position);
    ...
    async function save(id, row) {
      ...
      if (row) { row.style.transition = 'transform 160ms ease-out'; row.style.transform = 'translateX(-100%)'; }
      $('.notice').hidden = true;
      try {
        const result = await onSave(id);
        if (disposed) return;
        if (result.ok) { close(); onClose(result); return; }
      ...
      finally { busy = false; if (!disposed) { panel.removeAttribute('aria-busy'); render(); } }
    }
    function render() {
    ...
    swipe.addEventListener('pointermove',event => {
      ...
      if (gesture.row) {
        gesture.row.style.transition = 'none'; gesture.row.style.transform = `translateX(${Math.min(0,Math.max(dx,-swipe.clientWidth))}px)`;
    ...
    function finish(event,cancelled=false) {
      ...
      if (!cancelled && done.axis === 'x') {
        if (done.dx <= -threshold() && done.row) void save(done.row.dataset.classificationId,done.row);
        else if (done.dx >= threshold()) back();
      }
    }
    swipe.addEventListener('pointerup',event => finish(event));
    swipe.addEventListener('pointercancel',event => finish(event,true));
    swipe.addEventListener('lostpointercapture',event => finish(event,true));
...
.swipe{flex:1;min-height:0;position:relative;margin:8px 10px 8px 21px;touch-action:pan-y pinch-zoom;overflow:hidden}.rows{position:relative;height:100%;overflow-y:auto;overscroll-behavior:contain;scrollbar-width:thin;scrollbar-color:var(--ink) transparent;padding-right:5px;background:var(--paper)}
.row-wrap{position:relative;overflow:hidden;margin-bottom:4px;--row-paper:var(--paper)}.row-wrap:nth-child(even){--row-paper:#c9c5ac}.reveal{position:absolute;inset:0;display:flex;align-items:center;justify-content:flex-end;padding-right:16px;background:var(--quiet);color:var(--ink);font-size:12px}.ready .reveal{background:var(--ink);color:var(--paper)}
