...
final class CloudClient {
 ...
 private JSONObject authenticated(JSONObject s,String path,String method,JSONObject body,CancellationSignal cancel)throws Exception{
  ...
  HttpURLConnection c=(HttpURLConnection)new URL(s.getString("endpoint")+path).openConnection();
  try {
   ...
   ByteArrayOutputStream out=new ByteArrayOutputStream();try(InputStream in=c.getInputStream()){copy(in,out,4*1024*1024,cancel);}JSONObject result=new JSONObject(out.toString("UTF-8"));stripKeys(result);return result;
  } finally {if(cancel!=null)cancel.setOnCancelListener(null);c.disconnect();}
 }
