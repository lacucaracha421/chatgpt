...
public final class LibraryDocumentsProvider extends DocumentsProvider {
 ...
 static void invalidateMetadata(Context context){generation++;File dir=new File(context.getCacheDir(),"document-metadata");File[] files=dir.listFiles();if(files!=null)for(File f:files)if(f.isFile())f.delete();context.getContentResolver().notifyChange(DocumentsContract.buildChildDocumentsUri(AUTHORITY,ROOT),null);}
 ...
 private JSONObject recalled(String id,CancellationSignal cancel)throws Exception{// A cached document must still pass canonical visibility before it is exposed.
 JSONObject live=api("/v1/library/assets/"+Uri.encode(id)+"/media-ticket","POST",new JSONObject().put("variant","original"),cancel);
 File f=metaFile(id);if(!f.isFile()){JSONObject ticket=live;JSONObject a=new JSONObject().put("id",id).put("content_type",ticket.optString("content_type","application/octet-stream")).put("size_bytes",ticket.optLong("size_bytes",0)).put("thumbnail_available",false);remember(a);return a;}try(FileInputStream in=new FileInputStream(f)){ByteArrayOutputStream out=new ByteArrayOutputStream();CloudClient.copy(in,out,65536,null);return new JSONObject(out.toString("UTF-8"));}}
