    ...
    def test_resident_query_matches_legacy_and_reuses_reference_bundle(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            try:
                ...
                inspected = request("inspect_references", references=items[:5])
                self.assertEqual(inspected["type"], "references_inspected")
                ...
                self.assertEqual(legacy["referenceStatuses"], [row["state"] for row in inspected["items"]])
                self.assertEqual(legacy["referenceSelections"], [None] * 5)
