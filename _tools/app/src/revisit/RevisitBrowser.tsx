...
export function RevisitBrowser({ renderDay, onOpenCreator, onOpenBundle, privacyMode, cellSize, onSelectedDateChange }: {
  ...
  const navigation = <>
    ...
    {section === "browse" && <div className="revisit-browser__subtabs" role="tablist" aria-label="둘러보기 기준">
  ...
  if (section === "today") {
    return <section className="revisit-browser" aria-label="다시보기">
      <TodayView navigation={navigation} onOpenBundle={onOpenBundle} />
    </section>;
  }

  return <section className="revisit-browser" aria-label="다시보기">
    <ViewToolbar title="다시보기" ariaLabel="다시보기 도구" chrome={{ navigation }} />
