      ...
      {work.status === "running" ? (
        ...
        <div className="work-tray__result">
          ...
          {visibleMetadataDetails.map((failure) => {
            ...
            return <span className="work-tray__detail" aria-description={detail} key={detail}>{detail}</span>;
          })}
        ...
        <div className="work-tray__result">
          ...
          {work.failures.map((failure) => <span aria-description={failure.message} key={failure.fileName}>{failure.message}</span>)}
        ...
        <div className="work-tray__result">
          ...
          {work.exactDuplicates.map((item) => (
            <Button key={`${item.fileName}-${item.existingAssetId}`} variant="ghost" size="sm" aria-description={`${item.fileName} 기존 자산 열기`} onClick={() => openExisting(item.existingAssetId)}>
          ...
          {work.reviewPending.map((item) => <span aria-description={item.fileName} key={item.reviewId}>{item.fileName}</span>)}
          {work.failures.length > 0 && <div className="work-tray__failures">
            <ul>{work.failures.map((failure) => <li key={failure.fileName}><strong aria-description={failure.fileName}>{failure.fileName}</strong><span aria-description={failure.message}>{failure.message}</span></li>)}</ul>
