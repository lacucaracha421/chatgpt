...
/// subtree covers registered folders; ancestors cover images whose descendant
/// inference failed while they remain filed in their original parent folder.
...
pub(super) const FAILED_SCOPE_PREDICATE: &str = r#"EXISTS(
    SELECT 1 FROM asset_classifications ac WHERE ac.asset_id=j.asset_id AND (
        ...
        OR (ac.classification_id IN (SELECT id FROM ancestors)
...
pub(super) fn refresh_character_review_state(
    ...
    let series_ids = serde_json::to_string(
        &rows
            .iter()
    ...
    let accepted = connection
        .prepare(
            "SELECT r.target_id FROM character_relations r
             ...
             WHERE r.asset_id=?1 AND t.series_classification_id IN (SELECT value FROM json_each(?2)) ORDER BY r.target_id",
        )?
        .query_map(params![asset_id, series_ids], |row| row.get::<_, String>(0))?
        .collect::<std::result::Result<std::collections::BTreeSet<_>, _>>()?;
    let reference_known: bool = connection.query_row(
        "SELECT EXISTS(
            SELECT 1 FROM character_targets t
            WHERE t.series_classification_id IN (SELECT value FROM json_each(?2)) AND (
        ...
        params![asset_id, series_ids, content_hash],
        |row| row.get(0),
    )?;
...
impl Library {
    ...
    /// parent-folder assets whose job failed. A narrower scope would leave visible
    ...
    pub(super) fn character_autotag_context(
        ...
        let scope = json!({"classificationIds":job.classification_ids,"registeredSeriesIds":scope.series_classification_ids,"lineage":lineage,"seriesLineage":series_lineage,"invalidReferenceTargetIds":[]});
