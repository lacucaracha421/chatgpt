...
#[test]
fn unified_reference_selection_accepts_six_and_removes_either_kind() {
    let f = Fixture::new();
...
#[test]
fn unified_pool_keeps_all_twenty_five_after_legacy_anchor_removal() {
    let f = Fixture::new();
    let target = f.ready("Unified");
    ...
    assert_eq!(current.usable_references().count(), 25);
}
...
fn unified_reference_reselection_updates_changed_content_without_excluding_the_image() {
    ...
    let excluded: bool = f.library.connection().unwrap().query_row(
        "SELECT EXISTS(SELECT 1 FROM character_reference_exclusions WHERE target_id=?1 AND asset_id='asset-0')", [&target.id], |r| r.get(0),
    ).unwrap();
    assert!(!excluded);
}
...
impl Fixture {
    ...
    pub(in crate::library) fn ready_in_series(&self, name: &str, series: &str) -> Target {
        let target = self
            .library
            .save_character_target(TargetDraft {
                id: None,
...
fn registry_accepts_recursive_refs_and_rejects_invalid_atomic_replacement() {
    ...
    for invalid in [
        ...
        vec!["missing".into()],
        (0..26).map(|i| format!("asset-{i}")).collect(),
    ] {
        assert!(f
            .library
...
#[test]
fn moved_reference_is_excluded_without_blocking_remaining_references_or_competitors() {
    let f = Fixture::new();
    let a = f.ready("A");
    let _b = f.ready("B");
    f.decide(&a, &["asset-5"], DecisionKind::Accepted).unwrap();
