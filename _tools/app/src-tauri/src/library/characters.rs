...
impl Library {
    ...
    pub fn register_character_folder(
        ...
        if request.reference_ids.len() > MAX_REFERENCES {
            return Err(Error::Invalid("레퍼런스는 최대 25장입니다."));
        }
        ...
        let target = if let Some(id) = request.target_id {
            ...
            for (slot, image) in request.reference_ids.iter().enumerate() {
                ...
                if slot < REFERENCE_COUNT {
                    ...
                    tx.execute("INSERT INTO character_learned_references(target_id,asset_id,asset_hash,created_at) VALUES(?1,?2,?3,?4)", params![id,image,hash,now])?;
                }
            }
    ...
    pub fn exclude_character_reference(
        ...
        let removed = transaction.execute(
            ...
            "DELETE FROM character_references WHERE target_id=?1 AND asset_id=?2",
            params![id, asset_id],
        )?;
        if removed > 0 {
    ...
    pub(super) fn replace_character_references_selection_in(
        ...
        if asset_ids.len() > MAX_REFERENCES
            || asset_ids.iter().collect::<BTreeSet<_>>().len() != asset_ids.len()
        {
            return Err(Error::Invalid(
                "레퍼런스는 중복 없이 최대 25장까지 지정할 수 있습니다.",
            ));
        }
        let previous = self.read_character_target(transaction, id)?;
        ...
        for asset_id in asset_ids {
            ...
            values.push((asset_id.clone(), hash));
        }
        ...
        if !unchanged {
            ...
            for (removed_id, _) in previous_values.difference(&values.iter().cloned().collect()) {
                if !values.iter().any(|(asset_id, _)| asset_id == removed_id) {
                    transaction.execute("INSERT OR IGNORE INTO character_reference_exclusions(target_id,asset_id,created_at) VALUES(?1,?2,?3)", params![id,removed_id,now])?;
                }
            }
            transaction.execute(
