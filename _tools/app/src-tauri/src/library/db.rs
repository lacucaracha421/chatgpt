...
pub(crate) const SCHEMA_VERSION: i64 = 83;
...
fn migrate_to_latest(connection: &mut Connection, version: i64) -> Result<(), LibraryError> {
    ...
    let migration = (|| {
        ...
        if version <= 73 {
            transaction.execute_batch(include_str!("../../migrations/0074_mobile_publication.sql"))?;
        }
        if version <= 74 {
            transaction.execute_batch(include_str!("../../migrations/0075_mobile_catalog_visibility.sql"))?;
        }
        if version <= 75 {
