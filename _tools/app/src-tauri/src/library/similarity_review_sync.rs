...
impl Library {
    ...
    /// Claim the receive poll for this endpoint: true at most once a minute, durably.
    fn claim_similarity_review_poll(&self, endpoint: &str) -> Result<bool, LibraryError> {
        let db = self.connection()?;
        let due: bool = db
            .query_row(
                "SELECT last_checked <= unixepoch() - 60 FROM mobile_similarity_review_poll WHERE endpoint = ?1",
                [endpoint],
                |row| row.get(0),
            )
            .optional()?
        ...
        if due {
            db.execute(
                "INSERT INTO mobile_similarity_review_poll(endpoint, last_checked) VALUES (?1, unixepoch())
