...
impl Library {
    ...
    pub(crate) fn stop_character_scan(&self) {
        self.stop_character_incremental();
        self.character_scan
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .cancel
...
impl Library {
    pub(super) fn checked_character_evidence(
        ...
        rows.into_iter().map(|row| {
            ...
            Ok((row.asset_id, json!({"scanId":id,"runtimeFingerprint":status.runtime_fingerprint,"prediction":evidence,"references":target.usable_references().collect::<Vec<_>>()})))
        }).collect()
    }
}
