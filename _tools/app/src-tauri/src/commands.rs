...
impl From<LibraryError> for CommandError {
    fn from(error: LibraryError) -> Self {
        ...
        let code = match error {
            ...
            LibraryError::CatalogDuplicateUnsupported => "catalog_duplicate_unsupported",
            LibraryError::CatalogDuplicateSyncRejected(_) => "catalog_duplicate_sync_rejected",
            LibraryError::CatalogDuplicateInvalid => "catalog_duplicate_invalid",
            LibraryError::CatalogDuplicateCursorRejected => "catalog_duplicate_cursor_rejected",
            LibraryError::ReleaseSyncRejected(_) => "release_sync_rejected",
            LibraryError::ReleaseSyncInvalid => "release_sync_invalid",
            LibraryError::ReleaseReadCursorExpired(_) => "release_read_cursor_expired",
            LibraryError::ReleaseCursorRejected => "release_cursor_rejected",
            LibraryError::ReleaseGenerationStale => "release_generation_stale",
            LibraryError::BindingSyncRejected(_) => "binding_sync_rejected",
            LibraryError::BindingSyncInvalid => "binding_sync_invalid",
            LibraryError::BindingCursorRejected => "binding_cursor_rejected",
...
pub async fn notes_request(root:String, operation:String, input:serde_json::Value, app:tauri::AppHandle, state:State<'_,AppState>) -> Result<serde_json::Value,String> {
    ...
    tauri::async_runtime::spawn_blocking(move || -> crate::library::notes::Result<serde_json::Value> {
        ...
        Ok(match operation.as_str() {
            ...
            "unlockKeyring"=>serde_json::to_value(library.notes_unlock_keyring()?)?,
            "secretStatus"=>serde_json::to_value(library.notes_secret_status()?)?,
            "secretSetPin"=>serde_json::to_value(library.notes_secret_set_pin(input.get("pin").and_then(|v|v.as_str()).unwrap_or(""))?)?,
            "secretUnlock"=>serde_json::to_value(library.notes_secret_unlock(input.get("pin").and_then(|v|v.as_str()).unwrap_or(""))?)?,
