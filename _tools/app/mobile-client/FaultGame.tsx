...
export function FaultGame({items,onClose}:{items:readonly Asset[];onClose():void}) {
  ...
  const [state,setState]=useState<'loading'|'preparing'|'playing'|'failed'>('loading');
  const [progress,setProgress]=useState<{done:number;total:number}|null>(null);
  const scope=useRef(items),close=useRef(onClose);close.current=onClose;
  useEffect(()=>{
    const controller=new AbortController();
    const link=connectFaultFrame({game:()=>frame.current?.contentWindow??null,onClose:()=>close.current(),onLoaded:()=>setState(current=>current==='failed'?current:'playing')});
    // Older game builds never report loaded; don't leave the cover up forever.
    let fallback=0;
    void loadFaultPhotos(scope.current,controller.signal,(done,total)=>setProgress({done,total})).then(photos=>{
      if(controller.signal.aborted)return;
      if(!photos.length){setState('failed');return;}
      link.supply(photos);setState('preparing');
      fallback=window.setTimeout(()=>setState(current=>current==='preparing'?'playing':current),20000);
    },()=>{if(!controller.signal.aborted)setState('failed');});
    return()=>{controller.abort();link.dispose();window.clearTimeout(fallback);};
  },[]);
