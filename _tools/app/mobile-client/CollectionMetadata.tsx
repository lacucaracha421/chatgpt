...
import {displayGenres} from '../src/collections/displayGenres';
import {koreanGenreText} from '../src/collections/genreNames';
export function CollectionMetadata({item}:{item:CollectionDetail}) {
  const rows:[string,string|number|null|undefined][]=[
    // My rating is edited in the personal section, not repeated here.
    ...
    ['TMDB 평점',item.type==='movie'?item.externalScore:null],['장르',item.type==='manga'?koreanGenreText(item.genres):displayGenres(item.genres)],['상태',item.series?.status],
  ];
