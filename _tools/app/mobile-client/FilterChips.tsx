...
export function FilterChips({value,applied=value,onChange,open,onOpen,media=true,aspect=true,duration=true}:{value:AssetFiltersValue;applied?:AssetFiltersValue;onChange(value:AssetFiltersValue):void;open:FilterGroup|null;onOpen(group:FilterGroup|null):void;media?:boolean;aspect?:boolean;duration?:boolean}) {
  ...
  return <><div className="filter-chips" role="group" aria-label="자산 필터">{(Object.keys(groups) as FilterGroup[]).filter(key=>enabled[key]).map(key=>{
