...
import {Bars3Icon, BookOpenIcon, PhotoIcon, PencilSquareIcon, Squares2X2Icon, ArrowsPointingInIcon, ViewfinderCircleIcon, HomeIcon, RectangleStackIcon, AdjustmentsHorizontalIcon, ArrowPathIcon, ChevronRightIcon, XMarkIcon} from '@heroicons/react/24/outline';
...
export function App() {
  ...
  const [page, setPage] = useState<Committed>({items:[], has_more:false, next_cursor:null, view:LIBRARY, cursor:null, previous:[], version:0, restoreScroll:0, generation:null});
  ...
  const lastIntent = useRef<{view:View; cursor:string|null; previous:(string|null)[]}>({view:LIBRARY,cursor:null,previous:[]});
  ...
  useEffect(() => {
    ...
    const back = () => {
      ...
      else if (!isAll(state.page.view) || state.page.cursor) void load(LIBRARY);
  ...
  const openAll = () => {setArea('assets'); setDrawer(false); if(!isAll(latest.current.page.view)) void load(LIBRARY);};
  ...
  const openLibrary = () => {
    ...
    if(saved && !isAll(saved.view)) void load(saved.view,saved.cursor,saved.previous,saved.scroll);
    else openAll();
  };
  ...
  const updateStatus = (next: Status) => {
    ...
    setPage({generation:null,items:[],has_more:false,next_cursor:null,view:LIBRARY,cursor:null,previous:[],version:0,restoreScroll:0});
