...
function WorkCard({work,revision,active,meta=true,onOpen}:{work:CollectionSummary;revision:string;active:boolean;meta?:boolean;onOpen(id:string):void}) {
  const credit=collectionCardCredit(work),date=collectionCardDate(work);
  return <button className="collection-tile" onClick={()=>onOpen(work.id)}><Artwork item={work} id={collectionCover(work)} revision={revision} active={active}/><span className="collection-title">{work.name}</span>{credit&&<span className="collection-credit">{credit}</span>}{meta&&(date||work.myScore!=null)&&<span className="collection-card-meta">{date&&<span className="collection-date numeric">{date}</span>}{work.myScore!=null&&<Stars score={work.myScore}/>}</span>}</button>;
}
...
export function Collections({active,paused,backRef}:{active:boolean;paused:boolean;backRef:React.MutableRefObject<(()=>boolean)|null>}) {
  ...
  const listPull=usePullToRefresh(listRef,()=>setRefresh(n=>n+1),main.busy,!live||!!selected||showcaseAll);
  ...
  useEffect(()=>{
    ...
    setDetailError('');setCoverIndex(null);setOverview(false);setVolumeLimit(96);
  ...
  usePublicationCheck(live&&coverIndex===null,'/v1/collections/status',main.page?.revision,(_reply,changed)=>{if(!changed)return;setRefresh(n=>n+1);setDetailRefresh(n=>n+1);});
  ...
  const back=useCallback(()=>{
    ...
    return false;
  },[coverIndex,sheet,selected,showcaseAll]);
  useEffect(()=>{backRef.current=back;return()=>{backRef.current=null;};},[back,backRef]);
  useEffect(()=>{if(!active||paused)return;const key=(event:KeyboardEvent)=>{if(event.key==='Escape'&&coverIndex===null&&!sheet){if(back())event.preventDefault();}};window.addEventListener('keydown',key);return()=>window.removeEventListener('keydown',key);},[active,paused,back,coverIndex,sheet]);
  ...
  const openWork=(id:string)=>{if(listRef.current&&!showcaseAll)listScroll.current=listRef.current.scrollTop;setSheet(null);setSelected(id);};
  ...
  const description=item&&item.type!=='manga'?(item.overview||item.description):null;
  ...
  return <section className={`mobile-collections ${selected?'has-detail':''}`} style={{display:active?undefined:'none'}} aria-label="컬렉션">
    ...
    <div ref={listRef} className="collection-list" style={{display:selected||showcaseAll?'none':undefined}} onScroll={event=>{listScroll.current=event.currentTarget.scrollTop;if(nearEnd(event.currentTarget))main.loadMore();}}>
      ...
      {unpublished(main)?unpublishedNotice:<>
        {!filtered&&<section className="collection-showcase-fold" aria-label="쇼케이스">
          ...
          {showcaseOpen&&<div className="collection-shelf">{showcase.busy&&!showcase.items.length&&<p role="status" className="hint">쇼케이스를 불러오는 중…</p>}{showcase.error&&<p className="error-message" role="alert">{showcase.error}</p>}{!showcase.busy&&showcase.committed&&!showcase.items.length&&<p className="hint">쇼케이스에 고른 작품이 없습니다.</p>}{showcase.items.map(work=><WorkCard key={work.id} work={work} revision={showcase.page?.revision??''} active={live&&!selected} meta={false} onOpen={openWork}/>)}</div>}
        </section>}
        ...
        <div className={`collection-grid collection-grid-${type}`}>{main.items.map(work=><WorkCard key={work.id} work={work} revision={revision} active={live&&!selected} onOpen={openWork}/>)}</div>
    ...
    <div ref={showcaseRef} className="collection-list" style={{display:showcaseAll&&!selected?undefined:'none'}} onScroll={event=>{if(nearEnd(event.currentTarget))showcase.loadMore();}}>{showcaseAll&&<>
      ...
      {unpublished(showcase)?unpublishedNotice:<div className={`collection-grid collection-showcase collection-grid-${type}`}>{showcase.items.map(work=><WorkCard key={work.id} work={work} revision={showcase.page?.revision??''} active={live} meta={false} onOpen={openWork}/>)}</div>}
    ...
    <div ref={detailRef} className="collection-detail" style={{display:selected?undefined:'none'}}>{selected&&<>{detailPull}{detailError&&<div className="inline-error" role="alert">{detailError}<Button onClick={()=>setDetailRefresh(value=>value+1)}>다시 시도</Button></div>}{!item?(!detailError&&<p role="status" className="hint">작품을 불러오는 중…</p>):<>
      ...
      <div className={`collection-detail-intro ${background?'has-backdrop':''}`}>
        ...
        <div className="collection-detail-identity"><span className="collection-detail-kind">{labels[item.type]}</span><h1>{item.name}</h1>{maker&&<span className="collection-detail-credit">{makerLabels[item.type]} · {maker}</span>}
          <span className="collection-facts">{collectionCardDate(item)&&<span className="numeric">{collectionCardDate(item)}</span>}{item.myScore!=null?<Stars score={item.myScore}/>:<span>미평가</span>}{item.series?.status&&<span>{item.series.status}</span>}{item.platforms&&<span>{item.platforms}</span>}</span></div>
      </div>
