...
import {ChevronRightIcon,MagnifyingGlassIcon,PhotoIcon} from '@heroicons/react/24/outline';
...
export function LibraryRoot({entries,characters,recentFolders,items,total,paused,busy,revision,onSelect,onRefresh,albumTree,albumError,segment,onSegment,restoreScroll,onScroll}:{entries:Entry[];characters?:CharacterIndex;recentFolders:string[];items:Asset[];total?:number;paused:boolean;busy:boolean;revision:number;onSelect(view:View):void;onRefresh():void;albumTree:AlbumTree|null;albumError:string;segment:'folders'|'albums';onSegment(segment:'folders'|'albums'):void;restoreScroll:number;onScroll(top:number):void}) {
  ...
  return <><header className="library-root-header"><h1>라이브러리</h1></header><div className="library-root-scroll" ref={host} onScroll={event=>onScroll(event.currentTarget.scrollTop)} aria-label="라이브러리 탐색">{pull}
    ...
    <section><h2>분류</h2><FolderCards items={entries.filter(entry=>!entry.parent_id)} entries={entries} characters={characters} paused={paused} revision={revision} onSelect={onSelect}/>{!entries.length&&<p className="hint">아직 게시된 분류가 없습니다.</p>}</section>
    </>}
  </div></>;
}
