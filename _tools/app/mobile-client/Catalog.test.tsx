...
beforeEach(()=>{localStorage.clear();mocks.api.mockReset();mocks.native.mockReset();mocks.native.mockResolvedValue({url:'data:image/gif;base64,R0lGODlhAQABAAAAACw=',expires_in:240});mocks.api.mockImplementation(async(path:string)=>{
...
describe('mobile catalog reads',()=>{
  ...
  it('keeps reader chrome hidden while swiping, shows it only on tap, and supports pinch zoom',async()=>{
    ...
    fireEvent.pointerDown(stage,{pointerId:1,button:0,clientX:800,clientY:400});fireEvent.pointerMove(stage,{pointerId:1,clientX:650,clientY:400});fireEvent.pointerUp(stage,{pointerId:1,clientX:650,clientY:400});
    ...
    await waitFor(()=>expect((document.querySelector('.catalog-reader-slot.is-current img') as HTMLImageElement).style.transform).toContain('scale(1.5)'));expect(screen.getByRole('button',{name:'화면에 맞추기'})).toBeTruthy();
  });
  ...
  it('uses a ready total without issuing the extra count request',async()=>{
    ...
    await screen.findByText('1개');expect(mocks.api.mock.calls.some(([path])=>path.includes('/count?'))).toBe(false);
  });
