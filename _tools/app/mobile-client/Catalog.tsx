...
// The PC category ids/labels are the single source of truth; the mobile client must
...
export function Catalog({active,paused,backRef,endpoint=''}:{active:boolean;paused:boolean;backRef:MutableRefObject<(()=>boolean)|null>;endpoint?:string}){
  ...
  // The selector's category is an orthogonal filter, so it is committed as its own
  // query field alongside the text the user sees. The draft is preserved exactly,
  // because nothing here rewrites the user's own expression. `change` still resets
  // the cursor, caches, bookmarks, reader sequence and scroll exactly as any filter.
  function changeCategory(category:number|null){change({category});}
