...
Once the prototype has answered its question, capture the answer, then capture the prototype the way the [SKILL](SKILL.md) describes. The logic-specific mapping: the validated reducer / machine / function set lifts into the real module (the decision, absorbed); retain the self-contained HTML as a labeled prototype until an authorized retention/cleanup decision. Do not create a branch or commit as an implicit capture step; the parent skill and `AGENTS.md` govern promotion and Git authorization.
