# Lakomics Completed & Historical Work
...
- **Chrome 03b integration (`2d7bac2`, 2026-09-06).** Commit `2d7bac2` introduced the desktop archive UI/navigation chrome; same-day corrections refined labels, shell hints, selection, catalog controls, icon geometry and search placement. Evidence: TypeScript and focused React/Vitest coverage for shell, App, Asset toolbar, file ingestion, Manga/Online Catalog and shared menus; isolated Edge/Chromium fixture rendering at normal and narrow sizes; View Settings stayed inside the viewport without changing gallery width; Collection fixture search narrowed results. Not native production-library acceptance (file dialog, OS window, media protocol, active-library performance, every DPR/GPU path).
- **Paperback FINAL / case optimization (2026-09-06).** The final prototype was approved for application; renderer, virtual grids, appreciation strip and paged exhibition were integrated with no prototype artwork or fixture gateway shipped. TypeScript passed; the Collection suite passed 393 tests during integration and the affected 249-test subset after final changes; the native response helper passed 3 Rust tests. An isolated Edge 152 fixture with synthetic artwork showed 1,000-item lists keeping 28–42 card DOM nodes, one reusable WebGL book context, no live canvas per tile, and zero renders over two idle seconds; `drawGameCase.ts` was unchanged. The fixture covered 9/10/16/20-item exhibition boundaries, 800×640 and 1536×960, DPR 1.125/1.25/2, original-image fallback, context loss/restore, privacy unmount, previous/next volumes, Esc-to-opener focus and virtual-grid scroll/focus restoration; an 8px exhibition overflow and a frame/label overlap were fixed. The game-case jagged top edge was found at 154×231 CSS px, DPR 1.125, and visually improved at DPR 1.25. Native CORS/CSP was compiled/tested but not verified against the active library. The snapshot cache was memory-only at the time (limit 64 entries / ~24 MiB decoded); it later became the 256-entry / 64 MiB cache with IndexedDB persistence described in `works-viewer-design.md`.
- **Status/statistics additions (2026-09-06).** The rail then showed `동기화 문제 N` while failed queue items or direction-specific transport/metadata errors remained, opening Cloud Settings with public fixed messages only (PC-DECLUTTER-001 later moved it into the titlebar status center). Management gained `통계` with dense tables: inventory aggregates count normal Assets only, Collection totals all works, and metrics state scope, local-time basis and recorded-era limits. Collection opens are recorded once per uninterrupted detail session and never block viewing; v42 started Collection/daily telemetry without backfill (legacy Asset lifetime start is unknown); daily rows are bounded to 90 days with 30 shown. Storage inspection is opt-in, capped at 10,000 registered derivative paths, measures files outside the SQLite lock, shows partial/missing counts and labels original sizes as recorded sizes.
- **Manga ownership and release notifications (schema 41).** Schema 41 added ownership storage without inferring holdings or rewriting past notification states. The first UI had a toolbar unread inbox, a next-release-date line and a last-check/next-eligible line in the work sidebar; those were later replaced by the 신간 index row and omitted lines described in `lakomics-works-handoff-v2.md`.
...
delivery-record audit. It is not a new deployment, production audit or native acceptance.
`CLOUD-POST-001` remains active only for its explicitly separated residual scope;
`MEDIA-R2-001` and `CHAR-AUTO-001` are closed for the accepted current product scope.

### CLOUD-POST-001 — Completed authority slices

Status: `DONE` for the slices below, not for every future authority domain or cleanup.

- **Catalog bookmarks:** server authority, PC/Android durable intent and automatic
  convergence, including the runtime orchestration fix and production acceptance.
- **Albums:** authority-backed reads, PC reconciliation, Android membership writes and
  explicit conflict recovery; production activation and bidirectional membership
  acceptance are recorded. This does not claim Android structural editing.
- **Classifications:** server authority, client reconciliation and assignment flow,
  authority-backed tree reads, legacy-write fencing and adopted-client publication
  churn cleanup. Production activation and convergence are recorded.
- **Asset lifecycle:** server-owned normal/trash/tombstoned state, server-native Capture
  promotion and client adoption. ADR-0038 records production activation on 2026-09-19
  and the named lifecycle/device canary. Physical R2 garbage collection remains deferred.
- **Shared safety and client runtime:** per-domain identity/epoch/cursor/revision
  boundaries, durable queues where supported, idempotent commands, legacy-write fences,
  restore guards and automatic reconciliation for migrated domains.

Evidence: [ADR-0037](../adr/0037-server-authority-v2-replica-and-command-contract.md),
[ADR-0038](../adr/0038-asset-lifecycle-authority.md), authority cutover commit `0d5ab2d`,
and the dated bookmark/Album records below. Current PC runtime wires bookmark, Album,
Classification and Asset authority sync independently of remaining mobile publications.
The separately shipped mobile Character exclusion and PC acknowledgement are documented
in the [mobile Character contract](../agents/mobile-character-contract.md); they do not
transfer Character structures, reference management or inference authority to the server.

Remaining ownership decisions and safe publication/compatibility cleanup stay under
[CLOUD-POST-001](lakomics-backlog.md#cloud-post-001--서버-원천화-이후-클라이언트-구조-정리).
Do not interpret retained recovery/staging paths or PC-owned Character/Collection
publication as evidence that the completed core cutovers must be repeated.

### MEDIA-R2-001 — Current media delivery accepted

Status: `DONE` — the user is satisfied with current media performance and closes this
item at the delivered scope, rather than requiring the original expansion proposal.

Delivered: server-generated image/GIF/video-poster thumbnails at a maximum 512 px edge,
immutable derived thumbnail keys, improved video poster selection, source dimensions
and duration, historical metadata/thumbnail repair within its authorized scope, and
Android media caching. Deployment/device evidence remains in the dated
[MOBILE-UX-001 records](lakomics-backlog.md#mobile-ux-001--portrait-real-use-follow-up).
This closure does not override the narrower acceptance gaps recorded there.

**Not implemented and no longer required for this closure:** additional 256/1280/2048 px
variants, low-resolution motion/video derivatives, surface-size variant selection and
an expanded derivative cleanup lifecycle. The media contract remains `thumbnail` /
`original`; ordinary Viewer use can still fetch the original. Reopen a bounded
optimization only if a concrete latency, transfer or storage problem makes it useful;
do not automatically requeue the retired expansion under another ID.

### CHAR-AUTO-001 — Current accuracy-improvement pass

Status: `DONE` — closed for now at the user's request. This is a completed improvement
pass, not a claim that character recognition is solved or that all future mistakes
are acceptable.

Delivered scope:
- The existing incremental/quiet workflow, explicit supporting references and
  user-triggered historical refresh remain in place.
- Reference common-person inference and optional manual region correction reduce
  unnecessary person-selection work without replacing saved manual choices.
- Automatic confirmation still requires six distinct reference votes and a sixth
  distance at or below 0.16. A weak overlapping rival stops blocking only when its
  second distance is at least 0.05 worse than the winner's sixth distance; strong or
  close rivals and failed/missing competing evidence still block.
- The frozen evidence evaluator mirrors policy version 2 and retains version 1 replay.
  Manual judgments and self-reference protection remain authoritative.

Evidence from the implementation turn: 20 targeted Rust tests passed, with nine
external-environment cases ignored; 24 Python evaluator tests passed. Replaying the
reported image's eight saved candidates changed the result from unassigned to Monie
alone. A read-only pre-feedback/manual-label replay covered 869 pairs and added one
correct acceptance with no added false positives. The recent partition had only eight
pairs and usable PDQ fingerprints were absent, so this is not a general accuracy or
near-duplicate-screened holdout guarantee. Evaluator usage: [HOLDOUT.md](../../_tools/app/character-runtime/HOLDOUT.md).

Delivery limit: the latest local arbitration/reference changes were not established
as loaded in a running native app by these checks. A new native build/restart and
normal-use observation remain distinct from this product-scope closure; no production
reanalysis or classification writes were performed for the arbitration change, and no
new Windows acceptance is claimed.

Future false positives/negatives should be recorded as concrete cases in the living
backlog, with the asset, expected character, actual result and available evidence.
Continue improving this system when such cases arise; do not leave an indefinite
accuracy task active, weaken global gates from a single anecdote, restore a routine
review inbox, or start a full production backfill as an experiment.

### Historical CLOUD-POST-001 implementation checkpoints

Moved from the active item without rewriting the dated evidence. Statements such as
"not active" or "remaining" describe that checkpoint only; later activation and
acceptance records above and in the ADRs supersede them.
...
- `LONG-003` current VeraCrypt-backed `비밀` workflow is accepted as complete; the older Lakomics-native encrypted-container proposal is superseded, not implemented.
- `LONG-004` Display / Shelf mode is accepted as complete.
- `LONG-002B` focused cover interaction is accepted; future AV work is acquisition/selection UX under active `LONG-001`.
- `CHAR-UI-001` through `CHAR-UI-012` plus `CHAR-AUTO-002` / `CHAR-AUTO-005` are closed for the current product scope after sustained use. Remaining character concerns are accuracy (`CHAR-AUTO-001`) and batch-ingest visibility (`CHAR-AUTO-006`).
- `8b9a209` fixed reference membership beyond the first five images and Linux asset-viewer video playback; both were user-verified before commit/push.

## Archived detailed item records
...
Status: `DONE` — 2026-09-13 current-product user acceptance; older native-only gates are historical unless explicitly reopened.
...
- **빈 그룹 자동 해제 (2026-09-13):** 마지막 캐릭터가 이동·삭제되거나 그룹 편집에서 빠지면 그룹도 자동 해제된다. 스키마 72는 기존 빈 그룹을 정리하고 마지막 멤버 삭제 trigger를 둔다. 일반 멤버 교체 때는 새 멤버를 먼저 넣어 그룹이 중간에 삭제되지 않게 한다. 새 빈 그룹 생성은 막고, 활성 그룹의 마지막 멤버를 해제하면 시리즈로 돌아간다. 자산·캐릭터 삭제나 분석 예약은 하지 않는다. 그룹 교체·마지막 멤버 삭제·71→72 업그레이드와 활성 그룹 해제 회귀 통과; 네이티브 확인은 남음.
- **실제 시리즈 이동:** 별도 사용자 요청으로 구현됐으며 CHAR-UI-011의 참조·공유 자료·진행 중 작업 보호 계약을 따른다. 표시용 그룹 편집과 실제 시리즈 이동은 계속 구분한다.
...
## CHAR-UI-011 — 사이드바에서 캐릭터를 다른 시리즈로 이동

Status: `DONE` — 2026-09-13 current-product user acceptance; older native-only gates are historical unless explicitly reopened.

- **흐름:** 사이드바 캐릭터 우클릭 → `다른 시리즈로 이동…` → 등록된 대상 시리즈 선택 → 자료 수·공유 보관 위치 확인 → 이동. 시리즈 소속을 바꾸고 기존 그룹·일반 폴더 연결을 해제하며 새 위치로 탐색한다. 대상에 같은 이름의 다른 캐릭터가 있으면 거절한다.
- **공유 자료 수정:** 마커스 이동이 비활성화된 원인은 히사베스·로렌츠와 공유하는 일반 이미지 4장을 대상 시리즈에 넣으면 다른 캐릭터 화면에서 숨겨지는 보호 조건이었다. 이제 기존 폴더가 모든 연결에 적합하면 유지하고, 그렇지 않으면 대상 시리즈 또는 모든 연결을 보존할 수 있는 가장 가까운 공통 상위 폴더에 둔다. 마커스/라플라스처럼 형제 시리즈의 공유 이미지는 리버스에 보관해 양쪽 캐릭터·그룹에서 계속 표시한다. 확인창은 대상 시리즈 이동 수와 공유 자료 위치·이동 수를 나눠 표시한다.
- **보존·거절:** 같은 캐릭터 ID, 기준/추가 참조, 대표 이미지, 수동 판단·제외 이력과 원본 파일을 보존한다. 어느 캐릭터의 참조든 소속 시리즈 범위 안에 남아야 하며, 모두 보존할 위치가 없으면 이동을 막는다. 오래된 미리보기는 재확인을 요구하고 적용은 한 트랜잭션으로 처리한다.
- **분석 경계:** 미분류 스캔/과거 갱신/새 자동 작업을 시작하지 않는다. 이동 캐릭터의 기존 과거 갱신을 종료하고, 재배치 자산의 오래된 작업·claim과 해당 캐릭터의 오래된 명시적 스캔을 무효화한다. 대상 시리즈 자동 분류 설정, 명시적 제외와 무관한 대기 작업은 보존한다. 비게 된 그룹은 CHAR-UI-009에 따라 해제한다.
- **증거:** `_tools/app/src-tauri/src/library/character_series_move.rs`, `character_series_move_tests.rs`, `_tools/app/src/characters/CharacterSeriesMove.tsx` 및 테스트. Rust 10개·확인창 5개와 타입 검사 통과. 개발 바이너리 재빌드와 Vite의 변경 코드 제공은 확인했으나 실제 앱 내 이동 성공 증거로 취급하지 않는다.

## CHAR-UI-012 — 시리즈 일반 폴더 탐색과 폴더 단위 분류 제외

Status: `DONE` — 2026-09-13 current-product user acceptance; older native-only gates are historical unless explicitly reopened.

- 시리즈 개요에서 일반 직계 하위 폴더를 캐릭터·그룹 카드와 함께 표시한다. 폴더 아이콘/하위 썸네일을 사용하며 전체 카드가 최대 두 행과 같은 페이지 번호를 공유한다. 캐릭터와 연결된 일반 폴더는 별도 카드로 중복 표시하지 않는다.
- 일반 폴더 더보기에서 `캐릭터 분류에서 제외` / `캐릭터 분류에 다시 포함`을 제공한다. 스키마 71의 폴더 제외는 하위에 상속되고, 전체 갤러리의 자산은 보존하면서 미분류·자동 인식·참조 후보에서 제외한다. 이미 실행 중인 해당 범위의 오래된 작업은 무효화한다.
- 일반 자산/캐릭터 갤러리의 공통 우클릭 메뉴는 `출처 복사`·앨범 추가를 공유한다. 여러 선택 자료의 출처는 중복을 제거해 줄바꿈으로 복사하며 출처가 없으면 비활성화한다. 기존 우클릭 즐겨찾기·폴더 이동·선택 전체/해제 항목을 정리했다. 캐릭터 자체의 시리즈 이동 메뉴는 CHAR-UI-011이다.
- Rust 폴더/제외·업그레이드 회귀와 SeriesBrowser/CharacterManagement/AssetBrowser 프런트 회귀가 소스에 있다. 이번 문서 갱신은 해당 테스트를 새로 실행하거나 기존 네이티브 검증 범위를 넓힌 기록이 아니다.

## CHAR-AUTO-005 — 명시적 갱신으로 기존 미분류 전체 확인

Status: `DONE` — 2026-09-13 current-product user acceptance; older native-only gates are historical unless explicitly reopened.

- **동작:** 사용자가 갱신하면 해당 캐릭터의 등록 시리즈와 하위 일반 폴더 안에서 작업/admission 이력과 관계없이 적격 미분류의 ID·근거 포인터를 기존 refresh item에 확정한다. 상위 일반 폴더·형제 폴더·별도 등록된 하위 시리즈는 포함하지 않는다. 700장이 적격이면 즉시 전체 700장으로 안내한다. 원본 읽기·추론·모든 job 생성은 요청 시 하지 않고 기존 worker가 32장씩 공급한다. 참조 저장·캐릭터 등록·시리즈 이동은 갱신을 시작하지 않는다.
- **보호:** 폴더/자산 제외, 오리지널·휴지통·비이미지, 참조, 기존 수동 판단/분류 완료, 가장 가까운 시리즈 경계를 보존한다. 공급·claim 때 다시 검증하며 유효한 근거 재사용과 최종 저장 검증은 유지한다.
- **복구:** 동일 참조의 진행 중 요청은 합쳐지고 확정한 목록은 재시작·일시 정지·이후 수집에도 유지된다. 새 수집을 먼저 처리한다. 이전 cursor 요청을 자동으로 전체 갱신으로 확대하지 않는다. 이전 구현에서 예약된 상위 폴더 자료도 공급·분석 전 재검증에서 건너뛴다. 새 스키마는 없다.
- **범위 오류 수정 (2026-09-13):** 백합/카구야 갱신에 상위 만화 폴더 자료가 포함된다는 사용자 보고로, 신규 수집용 상위 폴더 추론을 명시적 과거 갱신에도 적용했던 오류를 수정했다. 요청·이전 cursor 발견·공급·분석 전 모두 같은 시리즈 subtree 경계를 사용한다. 신규 수집의 상위 폴더 추론은 유지한다. 수정 후 갱신 회귀 30 passed / 1 ignored, 신규 수집 상위 폴더 추론 회귀 2 passed / 1 ignored. 실행 중인 네이티브 앱 적용·운영 자료 재분석은 확인하거나 실행하지 않았다.
- **검증:** 이력 없는 700장 전부를 중복 없이 32장씩 공급하는 DB/큐 회귀, 제외·수동 판단·참조 보호, 재시작·이후 유입·일시 정지·새 수집 우선·진행 수량 회귀를 추가했다. 캐릭터 Rust 회귀 130 passed / 15 ignored; 동시 컬렉션 migration 73과 충돌한 기존 스키마 재개 테스트 2건은 제외했다. 마지막 범위 보호 보완을 포함한 refresh 검사 28 passed / 1 ignored, 프런트 4파일 46 passed, TypeScript 검사 통과. 700장 검사는 실제 모델 추론/운영 처리량 측정이 아니다.

## CHAR-AUTO-002 — 전체 진행률과 중복 재분석 성능/진단

Status: `DONE` — 2026-09-13 current-product user acceptance; older native-only gates are historical unless explicitly reopened.

- **현재 표시:** 실제 비교 중인 `시리즈 / 캐릭터`와 갱신 요청별 `처리 / 전체 · 남음 · 실패`를 보여준다. 요청한 캐릭터와 현재 경쟁 비교 중인 다른 캐릭터를 혼동하지 않는다. 새 이미지/수동 분석의 대기·처리 중 수는 따로 표시한다.
- **분모:** 이번 요청의 durable snapshot만 사용한다. 전역 누적 완료 수, 현재 한 이미지의 캐릭터 비교 수, 공급된 32장 큐를 전체 작업량으로 표시하지 않는다. 이전 cursor 요청의 전체가 미확정이면 `대상 확인 중`으로 표시한다.
- **복구:** 실패한 갱신은 이름·수량·재요청 안내를 유지하고, runtime 오류도 남은 수량을 가리지 않는다. 과거 갱신 pause/resume는 새 이미지 분석을 멈추지 않는다.
- **검증:** CHAR-AUTO-005와 함께 Rust 상태 집계·프런트 표시/제어를 검증했다. 아래 중복 분석 성능 기록은 과거 진단 자료이며 이번 UI 변경으로 해결됐다고 주장하지 않는다.
...
아래 EXT-010의 목표·단계·완료 기준은 2026-09-09 목록형 전환 당시 기록이다. 서버 소유 분류/프로필·페어링·저장 계약은 유지하지만, 반원/슬롯을 금지하는 옛 시각·입력 요구는 이후 구현된 EXT-011이 대체한다. 완료된 list-only 전환을 새 실행 계획으로 다시 잡지 않는다.
...
## LONG-003 — Private Vault (current product scope)

Status: `DONE` — 2026-09-13 user acceptance.

The shipped direction uses an externally mounted VeraCrypt/private volume. Lakomics discovers it, keeps portable metadata under `.lakomics`, exposes the conditional `비밀` destination, isolates external video playback, supports title/custom-thumbnail management, and reconciles the mounted vault.

### Superseded native-encryption proposal

The original wording below described a Lakomics-owned encrypted object/container format. That proposal is historical and **not required unfinished work**.

### LONG-003 — Superseded native-encryption proposal (historical wording)
...
# Historical audit / roadmap record
...
CATALOG-007A/B, IDEA-001/001A/001B, LONG-002A, WORKS-002, MOBILE-001/002/004/005/006 and EXT-005/006/007/008/009/010 are DONE.
IDEA-001B was user-accepted on 2026-09-12; its Windows acceptance remains unverified without reopening the completed product scope. New extension follow-ups EXT-011/012 remain VERIFY.
...
IDEA-001/001B, LONG-002A and WORKS-002. Do not schedule them again.

2026-09-13 character/extension follow-ups: CHAR-UI-009/011/012 and EXT-011/012 retain their native acceptance gates. CHAR-AUTO-005 full historical unclassified discovery and CHAR-AUTO-002 work-center progress are implemented with native acceptance pending. These items have no prerequisite on the independent lanes below.
...
2. **NOTE-001B — finish desktop Notes acceptance and documented remaining work.** NOTE-001A revision-safe server is DONE; do not reimplement it.
3. **CATALOG-002B — optional Heliotrope coexistence.** Not a prerequisite for the
   existing-provider Mobile catalog lane.
4. **WORKS-001 — remaining related-work/richer Film presentation.** TV/season/episode
   structure already exists; do not restart that foundation.
5. **LONG-001 / LONG-002B acceptance → LONG-004 — AV relations/cover roles and focused
...
6. **LONG-003 — HOLD:** threat model/format/recovery approval before encrypted
