...
Status: `PARTIAL` — portrait follow-up APK delivered; server image-thumbnail generation deployed, authorized recent-image repair verified, and repaired thumbnails confirmed visible by the user on Galaxy Tab. Remaining portrait interaction acceptance, shared Catalog visibility editing, video/GIF thumbnails and dimension rollout remain open.
...
- **Catalog hidden-tag controls:** expose a PC-like option to choose tags to hide/exclude from Catalog browsing. Check the existing PC semantics before deciding mobile behavior or whether preferences are shared across devices.
- **Catalog category controls:** expose selectable Catalog categories such as Artist CG and Doujinshi, following the actual PC category model rather than introducing a separate mobile taxonomy.
