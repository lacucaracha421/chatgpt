...
Review fixes (2026-09-24): a rebase or memo overwrite mints a new operation id (a changed payload always gets a new id); `operationConflict` triggers one reissue with a new id; one intent's refusal no longer stops other intents; a failed local save is reported instead of silently dropping the edit. Server: huge numbers get 422, and the replica PUT decides the caller's role before reading the body and parses it in the threadpool.
