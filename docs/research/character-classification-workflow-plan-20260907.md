...
## 2026-09-08 시리즈 탐색 개편 — 사용자 승인 구현 범위

- 시리즈는 폴더에서 명시적으로 등록한다. 게임/만화/기타 같은 이름이나 깊이를 동작 조건으로 사용하지 않는다.
- 사이드바는 기존 분류 트리 아래에 등록 캐릭터의 관계 조회 항목을 표시한다. 여러 캐릭터가 나온 에셋의 중복 파일을 만들지 않는다.
- 시리즈 본문: 선택한 hero art → 기본 폭 약 220px·세로 3:4 비율의 캐릭터 그리드 → 캐릭터 미분류 에셋의 수집일별 masonry. 캐릭터 카드 본체는 탐색, 정보 버튼은 편집 패널을 연다.
- 캐릭터 정보: 이름, 설명, 독립 대표 썸네일, 기준 이미지 최대 5장과 준비 상태. 설명과 대표 이미지 편집은 인식 fingerprint를 바꾸지 않는다.
- 높은 신뢰도는 자동 확정하고 애매한 결과는 검토한다는 사용자 선택을 반영한다. 기존 공개 인식 threshold는 유지하며, 자동 확정은 추가로 충분한 기준 일치와 후보 충돌 여부를 확인한다. 기존 사용자 판단을 덮어쓰지 않는다.
- 시리즈 수집과 상위 분류 직접 수집을 처리한다. 다른 시리즈의 하위 에셋은 비교 범위에 포함하지 않는다.
- 분석 진행/취소/재시도와 검토 진입을 현재 시리즈에서 제공한다.
- 순서: 저장/API → 탐색/편집 UI → 수집 후 분석/자동 확정 → 격리 Rust·React·브라우저 검증.
- Windows/Linux 공통 소스와 미디어 경로를 사용한다. 운영 라이브러리 적용, 실제 native acceptance, Git 쓰기는 별도 명시적 승인 범위다.

### 구현 및 검증 결과

- 위 시리즈 탐색·등록·편집·수집 후 자동 분류 흐름을 구현했다. DB v48은 시리즈 hero, 캐릭터 설명/대표 이미지, 자동 판단 출처를 저장한다.
- 자동 확정은 동일 crop에서 기준 이미지 3장 이상 일치하고 whole fallback이 없으며, 준비된 대상의 분석이 모두 완료된 경우에만 적용한다. 여러 캐릭터 후보 충돌, 기존 판단, 오류/오래된 결과는 자동 확정하지 않는다. 상위 분류에서 확정한 에셋은 시리즈로 이동하며 동기화 큐와 판단 출처를 함께 기록한다.
- TypeScript 검사 통과. 관련 React/갤러리/사이드바 테스트 102개, Rust 캐릭터 테스트 18개, DB migration 테스트 47개 통과. 실제 모델 환경이 필요한 Rust 테스트 3개는 건너뛰었다.
- 별도 App 테스트 확장 실행에서는 기존 back navigation 및 appZoom 기대값과 관련한 실패 2개가 남았다. 해당 범위의 기존 변경은 수정하지 않았다.
- 격리 브라우저 fixture에서 큰 세로 카드, 정보/기준 이미지 패널, 설명 저장, 연속 스크롤 및 800px 너비를 확인했다. 미리보기 이미지는 합성 fixture다.
- 실제 Windows/Linux Tauri 실행, 실제 모델 인식, 운영 DB v48 적용은 검증하지 않았다. 운영 라이브러리에는 쓰지 않았다.
