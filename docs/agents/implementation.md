...
- 현재 앱 패키지는 저장소 루트 기준 `_tools/app/`입니다. React 코드는 `src/`, Rust 코드는 `src-tauri/`, 공유 모바일 프런트엔드는 `mobile-client/`에 있습니다.
- npm 명령은 `_tools/app/`, Cargo 명령은 `_tools/app/src-tauri/`에서 실행해 해당 패키지의 설정과 고정 도구 버전을 사용합니다. dev 실행과 플랫폼별 체크아웃 경로는 루트 `AGENTS.md`를 따릅니다.
- 과거 문서의 `app/` 경로를 현재 실행 위치로 가정하지 않습니다. 파일 이동 시 README의 상대 링크, 보조 스크립트의 입력·출력 경로도 실제 패키지와 대조합니다.
