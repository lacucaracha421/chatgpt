...
impl Library {
    fn character_conversion_in(
        ...
        let series = target
            ...
            .as_deref()
            .ok_or(Error::Invalid("시리즈를 먼저 연결해 주세요."))?;
        ...
        if let Some(id) = &destination {
            ...
            if special {
                return Err(Error::Invalid("같은 이름의 폴더가 다른 시리즈나 캐릭터에 연결되어 있습니다. 먼저 폴더 이름을 변경해 주세요."));
            }
        }
        let ids = connection.prepare("SELECT asset_id FROM character_relations WHERE target_id=?1 UNION SELECT asset_id FROM character_references WHERE target_id=?1 AND asset_id IS NOT NULL UNION SELECT asset_id FROM character_learned_references WHERE target_id=?1 ORDER BY asset_id")?.query_map([target_id],|r|r.get::<_,String>(0))?.collect::<std::result::Result<Vec<_>,_>>()?;
        let mut shared = 0;
        let mut unavailable = target
            .references
            .iter()
            .filter(|r| r.asset_id.is_none())
            .count();
        let mut snapshot = Vec::new();
        for id in &ids {
            let (status, hash): (String, String) = connection.query_row(
    ...
    pub fn character_conversion_preview(&self, target_id: &str) -> Result<ConversionPreview> {
        let connection = self.connection()?;
        self.character_conversion_in(&connection, target_id)
    }
    pub fn convert_character_to_folder(
        &self,
        target_id: &str,
        token: &str,
        confirmation: &str,
        ...
        if confirmation != preview.name {
            return Err(Error::Invalid("캐릭터 이름을 정확히 입력해 주세요."));
        }
        let destination = if let Some(id) = preview.destination_id {
            id
        } else {
        ...
        tx.execute(
            "DELETE FROM character_decisions WHERE target_id=?1",
            [target_id],
        )?;
