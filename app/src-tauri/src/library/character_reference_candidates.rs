...
pub(super) fn reference_set_hash(target: &Target) -> Result<String> {
    let anchors = target
        .references
        .iter()
        .map(|reference| json!([reference.slot, reference.asset_id, reference.asset_hash]))
        .collect::<Vec<_>>();
    let learned = target
        .learned_references
        .iter()
    ...
    Ok(Sha256::digest(serde_json::to_vec(
        &json!({"anchors": anchors, "learned": learned}),
    )?)
    .iter()
...
impl Library {
    pub fn reference_candidates(
        ...
        let remaining = if target.manual_only {
            limit
        } else {
            limit.min(20usize.saturating_sub(target.learned_references.len()))
        };
            ...
            minimum_selection: if confirmation_mode == ReferenceConfirmationMode::Initialize {
                5usize.saturating_sub(
                    target
                        ...
                        .iter()
                        .filter(|reference| {
                            reference.status == "ready" && reference.asset_id.is_some()
                        })
                        .count(),
                )
            } else {
    ...
    pub fn confirm_reference_batch(&self, request: ConfirmReferenceBatch) -> Result<Target> {
        ...
        let result = match request.confirmation_mode {
            ...
            ReferenceConfirmationMode::AddLearned => {
                if target.manual_only || !target.ready {
                    return Err(Error::Invalid("기준 레퍼런스 5장을 먼저 설정해 주세요."));
                }
                self.add_character_learned_references_in(
                    &transaction,
                    &target.id,
