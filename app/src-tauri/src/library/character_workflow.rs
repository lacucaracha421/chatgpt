...
impl Library {
    ...
    pub fn complete_character_review(
        ...
        let registered: bool = transaction.query_row(
            "SELECT EXISTS(SELECT 1 FROM character_series WHERE classification_id=?1)",
            [&request.series_id], |row| row.get(0),
        )?;
        if !registered {
            return Err(Error::Invalid("등록된 시리즈를 선택해 주세요."));
        }
...
mod tests {
    ...
    fn review_completion_hides_only_the_current_partial_generation() {
        ...
        let needs_review = || f.library.browse_character_assets(character_hub::BrowseQuery {
            series_id: f.series.clone(), target_id: None, group_id: None, reference_target_id: None,
            after: None, limit: 100, all: false,
        ...
        assert_eq!(f.library.complete_character_review(CharacterReviewCompletionRequest {
            series_id: f.series.clone(), asset_ids: vec!["asset-5".into()],
        }).unwrap(), 1);
        assert!(needs_review().items.is_empty());
        assert_eq!(f.library.character_relations_for_asset("asset-5").unwrap(), relations);
        let all = f.library.browse_character_assets(character_hub::BrowseQuery {
            series_id: f.series.clone(), target_id: None, group_id: None, reference_target_id: None,
            after: None, limit: 100, all: true,
    ...
    fn review_completion_is_atomic_and_a_relation_change_reopens_review() {
        ...
        assert!(f.library.complete_character_review(CharacterReviewCompletionRequest {
            series_id: f.series.clone(), asset_ids: vec!["asset-5".into(), "asset-6".into()],
        }).is_err());
        assert_eq!(f.library.connection().unwrap().query_row(
            "SELECT COUNT(*) FROM character_review_completions", [], |row| row.get::<_, i64>(0),
        ).unwrap(), 0);

        f.library.complete_character_review(CharacterReviewCompletionRequest {
            series_id: f.series.clone(), asset_ids: vec!["asset-5".into()],
        }).unwrap();
        f.library.record_character_decisions(DecisionRequest {
            target_id: target.id, expected_fingerprint: target.fingerprint,
            asset_ids: vec!["asset-5".into()], decision: DecisionKind::Cleared,
            baseline_fingerprint: None, scan_id: None,
        }).unwrap();
        assert_eq!(f.library.connection().unwrap().query_row(
            "SELECT COUNT(*) FROM character_review_completions WHERE asset_id='asset-5'", [], |row| row.get::<_, i64>(0),
        ).unwrap(), 0);
    }
