...
const TARGET_GALLERY_SQL: &str = r#"
WITH RECURSIVE scope(id) AS (
    ...
    WHERE a.status='normal' AND (
        ...
        OR (d.accepted=1 AND EXISTS(SELECT 1 FROM asset_classifications ac WHERE ac.asset_id=a.id AND ac.classification_id IN (SELECT id FROM ancestors WHERE parent_id IS NULL)))
    )
    ...
    pub fn browse_character_assets(&self, query: BrowseQuery) -> Result<BrowsePage> {
        ...
        let (ids, total) = {
            ...
            if let Some(group_id) = query.group_id.as_ref() {
                ...
                let scope = if query.target_id.is_some() { scoped_references.as_str() } else { reference_scope };
...
mod tests {
    ...
    fn series_gallery_separates_unclassified_from_partially_resolved_assets() {
        ...
        let unclassified = f.library.browse_character_assets(query(SeriesGalleryFilter::Unclassified, false)).unwrap();
        assert!(unclassified.items.is_empty());
        let needs_review = f.library.browse_character_assets(query(SeriesGalleryFilter::NeedsReview, false)).unwrap();
        assert_eq!(needs_review.items.iter().map(|asset| asset.id.as_str()).collect::<Vec<_>>(), vec!["asset-5"]);
        let all = f.library.browse_character_assets(query(SeriesGalleryFilter::All, true)).unwrap();
    ...
    fn target_gallery_keeps_root_moved_relations_and_drops_stale_membership() {
        ...
        let query = || BrowseQuery {
            series_id: f.series.clone(), target_id: Some(target.id.clone()), group_id: None,
    ...
    fn group_gallery_unions_member_assets_without_duplicates() {
        ...
        let page = f.library.browse_character_assets(BrowseQuery { series_id:f.series.clone(), target_id:None, group_id:Some(group_id.clone()), reference_target_id:None, after:None, limit:100, all:false, series_filter:None }).unwrap();
        ...
        assert!(f.library.browse_character_assets(BrowseQuery { series_id:f.outside.clone(), target_id:None, group_id:Some(group_id), reference_target_id:None, after:None, limit:100, all:false, series_filter:None }).is_err());
    }
    #[test]
    ...
    fn reference_picker_excludes_other_characters_before_pagination() {
        ...
        let query = |id: &str, after| BrowseQuery {
            ...
            after, limit: 2, all: true, series_filter: None,
        };
