...
export function CharacterLab({ classifications, initialSeriesId, targetId, refreshVersion = 0, privacyMode = false, onClose, onEdit, api = characterApi }: { classifications: ClassificationEntry[]; initialSeriesId: string | null; targetId: string; refreshVersion?: number; privacyMode?: boolean; onClose: () => void; onEdit?: () => void; api?: CharacterApi }) {
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        if (stamp !== terminal.current) {
          terminal.current = stamp; setRefresh(v => v + 1);
    ...
    void poll();
    return () => { active.current = false; clearTimeout(timer); ++generation.current; };
  }, [api, reloadTargets]);
  ...
  useEffect(() => { setPage({ rows: [], nextCursor: null }); setSelection(emptySelection()); if (seriesId) void load(); }, [scopeKey, refresh, refreshVersion, api]);
  ...
  async function scan() {
    ...
    await action(async () => {
      ...
      if (active.current) {
        ...
        setNotice("분류를 시작했습니다. 이 창을 닫아도 계속 진행합니다.");
      }
    });
  }
  function cancel() {
    ...
    <div className="character-lab" aria-busy={busy}>
      ...
      <main className="character-lab__main">
        <div className="character-review-toolbar">
          ...
          <div className="character-review-toolbar__actions"><Button size="sm" variant="ghost" disabled={busy || loading} onClick={() => setRefresh(v => v + 1)}>새로고침</Button><Button size="sm" disabled={!target?.ready || !runtimeReady || busy || Boolean(scanning)} onClick={() => void scan()}>분류 시작</Button>
        ...
        {history ? <div className="character-history">
          ...
          <div className="character-lab__workspace">
            ...
            <div className="character-evidence-slot">{focused && <CharacterEvidence key={focused.asset.id} row={focused} privacyMode={privacyMode} busy={busy} onDecide={(p, decision) => void action(() => decideRows([focused], p, decision))} targetId={targetId} />}</div>
          </div>
        </>}
      </main>
    </div>
