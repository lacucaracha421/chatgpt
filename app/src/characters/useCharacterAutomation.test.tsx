...
it("refresh and target changes never launch target scans or create native work", async () => {
 ...
 const changed = vi.fn();
 const { rerender } = renderHook(({version}) => useCharacterAutomation([fixtureTarget()],[],version,changed,api),{initialProps:{version:0}});
...
it("publishes one refresh per durable completion and does not cancel on navigation", async () => {
 ...
 const changed=vi.fn();const {unmount}=renderHook(()=>useCharacterAutomation([],[],0,changed,api));
 await act(async()=>{await vi.advanceTimersByTimeAsync(100);});
...
it("pause and resume control the native queue without renderer-owned scans",async()=>{
 ...
 const {result}=renderHook(()=>useCharacterAutomation([],[],0,vi.fn(),api));
