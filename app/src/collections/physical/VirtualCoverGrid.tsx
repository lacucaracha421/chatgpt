...
type Props<T> = { items:readonly T[]; itemKey:(item:T)=>string; render:(item:T,index:number)=>ReactNode; label:string; scrollRef?:RefObject<HTMLDivElement|null>; centered?:boolean; className?:string; metadataHeight?:number; legacyMetrics?:boolean; onEmptyContextMenu?:()=>void };
export function VirtualCoverGrid<T>({items,itemKey,render,label,scrollRef,centered=false,className="",metadataHeight=42,legacyMetrics=false,onEmptyContextMenu}:Props<T>) {
  ...
  const columns=centered?Math.min(availableColumns,Math.max(1,items.length)):availableColumns;
  const cellWidth=centered?Math.min(176,(width-gap*(columns-1))/columns):(width-gap*(columns-1))/columns;
  const rowHeight=cellWidth*(legacyMetrics?1.5:368/256)+metadataHeight+gap,rows=Math.ceil(items.length/columns),virtual=items.length>64;
  ...
  const rowStyle={gridTemplateColumns:`repeat(${columns},minmax(0,1fr))`,width:centered?Math.min(width,columns*cellWidth+(columns-1)*gap):undefined} satisfies CSSProperties;
  ...
  return <div ref={ref} className={`virtual-cover-grid ${legacyMetrics ? "virtual-cover-grid--legacy" : ""} ${className}`} role="group" aria-label={label} onKeyDown={handleKey}
