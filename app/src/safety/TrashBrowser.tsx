...
export function TrashBrowser() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [nextPage, nextPolicy] = await Promise.all([
        gateway.listTrash({ after: null, limit: ASSET_PAGE_SIZE }),
        gateway.getTrashPolicy(),
      ]);
      setPage(nextPage);
      setPolicy(nextPolicy);
      setRetentionDays(nextPolicy.retentionDays?.toString() ?? "");
    } catch (error) {
      setMessage(commandErrorMessage(error, "휴지통을 불러오지 못했습니다."));
    } finally {
      setLoading(false);
    }
  }, [gateway]);
  ...
  return <section className="trash-browser" aria-label="휴지통">
    <header className="trash-browser__toolbar">
      ...
      <Button variant="danger" onClick={() => setConfirmEmpty(true)} disabled={!page || page.totalCount === 0}>휴지통 비우기</Button>
    </header>
    <section className="trash-browser__policy" aria-label="보존 기간 설정">
      <Toggle checked={automaticDeletion} disabled={!policy} onChange={(event) => void setAutomaticDeletion(event.target.checked)}>자동 삭제</Toggle>
      {automaticDeletion && <div className="trash-browser__retention"><TextField label="보존 기간" type="number" min={MIN_RETENTION_DAYS} max={MAX_RETENTION_DAYS} value={retentionDays} error={retentionError} onChange={(event) => setRetentionDays(event.target.value)} /><Button onClick={() => void saveRetention()} disabled={Boolean(retentionError)}>저장</Button></div>}
    </section>
    {message && <Toast>{message}</Toast>}
    {loading ? <Skeleton className="trash-browser__skeleton" label="휴지통을 불러오는 중" /> : !page || page.items.length === 0 ? <EmptyState title="휴지통이 비어 있습니다">삭제한 자산은 이곳에서 복원할 수 있습니다.</EmptyState> : <ul className="trash-browser__list">{page.items.map(({ asset, trashedAt, purgeAt }) => <li key={asset.id} className="trash-browser__item"><div><strong>{asset.title || asset.originalName}</strong><span>삭제: {localDate(trashedAt)}</span><span>{purgeAt ? `영구 삭제까지 ${remainingDays(purgeAt)}일` : "자동 삭제 안 함"}</span></div><Button onClick={() => void restore(asset.id)}>복원</Button></li>)}</ul>}
    {confirmEmpty && page && <Dialog open title="휴지통 비우기" onClose={() => setConfirmEmpty(false)}><p>휴지통의 자산 {page.totalCount}개 ({formatBytes(page.totalBytes)})를 영구 삭제합니다.</p><p>이 작업은 되돌릴 수 없습니다.</p><div className="ui-dialog__actions"><Button onClick={() => setConfirmEmpty(false)}>취소</Button><Button variant="danger" disabled={emptying} onClick={() => void emptyTrash()}>영구 삭제</Button></div></Dialog>}
  </section>;
}
