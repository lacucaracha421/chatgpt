...
function Cover({asset, paused}: {asset:Asset; paused:boolean}) {
  ...
  return <span className="home-cover">{preview ? <img src={preview} alt="" loading="lazy" draggable={false}/> : <PhotoIcon className="missing-media"/>}{asset.kind === 'video' && <span className="video-mark"><PlayIcon/>영상</span>}</span>;
}
...
export function Home({items,classifications,recentFolders,revisit,captures,busy,paused,secondaryError,revision,onSelect,onOpen,onPending}:HomeProps) {
  ...
  const recent = recentFolders.map(id => classifications.find(folder => folder.id === id)).filter((folder):folder is Classification => !!folder);
  ...
  return <div className="home-scroll" aria-label="홈 탐색">
    <section aria-label="최근 저장"><div className="home-section-heading"><span className="hint">새로 보관한 이미지와 영상</span><Button variant="ghost" onClick={() => onSelect({tab:'library',title:'최근 저장'})}>전체 보기<ArrowUpRightIcon/></Button></div>
      {items.length ? <div className="home-recent-strip">{items.slice(0,12).map((asset,index) => <button className="home-asset" key={asset.id} onClick={() => onOpen(index)} aria-label={`${asset.creator_name || asset.creator_handle || (asset.kind === 'video' ? '영상' : '이미지')}, ${dateLabel(asset)}`} data-asset-id={asset.id}><Cover asset={asset} paused={paused}/><span className="tile-caption"><span>{asset.creator_name || asset.creator_handle || '작가 정보 없음'}</span><time>{dateLabel(asset)}</time></span></button>)}</div> : <p className="home-empty">{busy ? '최근 저장을 불러오는 중…' : '아직 동기화된 자산이 없습니다.'}</p>}
    </section>
    {!!recent.length && <section className="home-recent-folders" aria-label="최근 방문 분류"><h3>최근 방문</h3><div>{recent.map(folder => <Button variant="ghost" key={folder.id} onClick={() => openFolder(folder)} title={folderBreadcrumb(folder,classifications)}><ClassificationIcon kind="tag" iconKey={folder.icon_key ?? null} style={{color:classificationColor(folder.color_key ?? null)}}/><span>{folderBreadcrumb(folder,classifications)}</span></Button>)}</div></section>}
